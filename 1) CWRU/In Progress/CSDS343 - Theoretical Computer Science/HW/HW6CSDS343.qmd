...
stateDiagram
    [*] --> A: epsilon, epsilon -> $
    A --> B: epsilon, epsilon -> epsilon
    A --> A: a, epsilon -> a
    B --> Accept: epsilon,  a -> epsilon
    B --> B: b, a -> epsilon
    B --> B*: b, $ -> b
    B* --> B*: b, epsilon -> b
    B* --> Accept: epsilon, b -> epsilon
```
...
stateDiagram
    ...
    A --> B: epsilon, epsilon -> epsilon
    B --> Accept: epsilon, epsilon -> epsilon
```

> $A$ is the machine for $L_1$, $B$ is the machine for $L_2$
>
> We map the accept state of the first automata to the initial state of the second automata.
>
