...
MI and $\chi^2$ both reject the null of independence at α = 0.05 (p < 0.01).

JI = 0 means both columns almost never share 1’s simultaneously; the dependence found by MI/$\chi^2$ must therefore involve other value patterns (i.e, non-co-occurrence of ones, or correlation of zeros).
...
MI and $\chi^2$ are both sensitive to any dependence between two categorical variables, not just co-occurrence of 1’s.
...
The additional 36–38 pairs that are MI/$chi^2$-only likely represent inverse or asymmetric dependencies (e.g., one variable tends to be 0 when the other is 1).
